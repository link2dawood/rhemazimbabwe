...
class Exam extends MY_Addon_CBSEController
{
    ...
    public function addexamsubject()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $not_be_del = array();
            $assessment_array = [];
            $prev_assessment_array = $this->input->post('prev_assessments');
            if (!isset($prev_assessment_array)) {
                $prev_assessment_array = [];
            }

            $rows = $this->input->post('rows');
            $timetable_prev_rows = $this->input->post('prev_row');

            foreach ($rows as $row_key => $row_value) {
                $assessment_array['assessment_' . $row_value] = $this->input->post('assessment_' . $row_value);
            ...
            $this->cbseexam_exam_model->add_examsubject($insert_array, $update_array, $not_be_del, $this->input->post('exam_id'), $timetable_prev_rows, $assessment_array, $prev_assessment_array);
    ...
    public function teacherRemark()
    {
        ...
        $subjects         = $this->cbseexam_exam_model->getexamsubjects($data['exam_id']);
        $data['subjects'] = $subjects;
        $data['exam_subject_assessments'] = $this->cbseexam_assessment_model->getSubjectAssessmentsByExam($subjects);
        $total_exam_marks = 0;

        if (!empty($data['exam_subject_assessments'])) {
            foreach ($data['exam_subject_assessments'] as $exam_subject_key => $exam_subject_value) {
                if (!empty($exam_subject_value->subject_assessments)) {
                    foreach ($exam_subject_value->subject_assessments as $assessment_key => $assessment_value) {
                        $total_exam_marks += $assessment_value->maximum_marks;
                    }
                }
            }
        }
                ...
                foreach ($_POST['mark'][$exam_student_id] as $assement_key => $assement_value) {
                    ...
                    if (isset($_POST['absent'][$exam_student_id][$assement_key])) {
                       ...
                       $marks=  0;
                        ...
                        $marks=  $assement_value['marks'];
                    }

                    $insert_array[] = array(
                        'cbse_exam_timetable_assessment_type_id' => $assement_value['cbse_exam_timetable_assessment_type'],
    ...
    public function addattendance()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($exam_id) {
                ...
                $this->cbseexam_exam_model->add($examdata,false);
    ...
    public function edit()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $exam_detail = $this->cbseexam_exam_model->get_exambyId($this->input->post('exam_id'));
            $assessment_delete = false;
            if ($exam_detail['cbse_exam_assessment_id'] != $this->input->post('assessment_id')) { //============if assessment value changed delete old record
                $assessment_delete = true;
            }

            $this->cbseexam_exam_model->add($data, $assessment_delete);
