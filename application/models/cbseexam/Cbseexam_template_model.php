...
class Cbseexam_template_model extends MY_Model
{
    ...
    */
    ...
    public function delete_img($data)
    {
        ...
        if ($data['img_type'] == "left_sign") {
            $data['left_sign'] = NULL;
        } elseif ($data['img_type'] == "right_sign") {
            $data['right_sign'] = NULL;
        } elseif ($data['img_type'] == "middle_sign") {
            $data['middle_sign'] = NULL;
        } elseif ($data['img_type'] == "background_img") {
            $data['background_img'] = NULL;
        } elseif ($data['img_type'] == "header_image") {
        ...
        $message = UPDATE_RECORD_CONSTANT . " On cbse template id " . $data['id'];
        $action = "Update";
        $record_id = $data['id'];
    ...
    public function all_term($template_id)
    {
        ...
        $this->db->select('cbse_terms.*')->from('cbse_exam_class_sections')->join('cbse_template_class_sections', 'cbse_template_class_sections.class_section_id=cbse_exam_class_sections.class_section_id')->join('cbse_exams', 'cbse_exams.id=cbse_exam_class_sections.cbse_exam_id')->join('cbse_terms', 'cbse_terms.id=cbse_exams.cbse_term_id')->where('cbse_exams.session_id', $this->current_session)->where('cbse_template_class_sections.cbse_template_id', $template_id)->where('cbse_exams.session_id', $this->current_session);
        ...
        $result = $this->db->group_by('cbse_terms.id')->get()->result_array();
       
        $return = array();
        foreach ($result as $key => $value) {
            $this->db->select('cbse_exams.*')->from('cbse_exams')->join('cbse_exam_class_sections', 'cbse_exams.id=cbse_exam_class_sections.cbse_exam_id')->where('cbse_term_id', 	$value['id'])->where('cbse_exams.session_id', $this->current_session);
			...
			$exam =  $this->db->group_by('cbse_exams.id')->get()->result_array();
