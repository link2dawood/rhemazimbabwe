<?php
// Test Partner Module Lib Fix
// Run: http://localhost/rhemazimbabwe/test_partner_module_lib_fix.php

echo "<h1>Testing Partner Module Lib Fix</h1>";
...
echo "<h2>2. Testing Module Lib Loading</h2>";

try {
    // Test if module_lib library exists
    if (file_exists(APPPATH . 'libraries/Module_lib.php')) {
        echo "<p style='color: green;'>✅ Module_lib library file exists!</p>";
    } else {
        echo "<p style='color: red;'>❌ Module_lib library file not found!</p>";
        exit;
    }
    
    // Load module_lib library
    ...
    if (method_exists($CI->module_lib, 'hasModule')) {
        ...
        echo "<p>Online Course module status: " . ($has_online_course ? 'Available' : 'Not Available') . "</p>";
    } else {
        echo "<p style='color: red;'>❌ hasModule() method not found!</p>";
    }
    
} catch (Exception $e) {
    echo "<p style='color: red;'>❌ Error loading module_lib: " . $e->getMessage() . "</p>";
...
foreach ($routes_to_test as $route => $description) {
    ...
    echo "<td>Should load without module_lib errors</td>";
    echo "</tr>";
}
echo "</table>";
...
echo "<ul>";
echo "<li><strong>No Module Lib Errors:</strong> Partner pages should load without 'Undefined property: CI_Loader::$module_lib' errors</li>";
echo "<li><strong>Header Functions:</strong> hasModule() and other module_lib methods should work</li>";
...
echo "<ul>";
echo "<li><strong>Library Loading:</strong> Added module_lib loading to Partner controller</li>";
echo "<li><strong>View Data:</strong> Pass module_lib to all views via \$data array</li>";
echo "<li><strong>Method Availability:</strong> hasModule() and other methods now available</li>";
echo "</ul>";
echo "</div>";
...
echo "<ul>";
echo "<li>Click Partners menu - should load without module_lib errors</li>";
echo "<li>Check that student header displays correctly</li>";
echo "<li>Click 'Add Partner' - should show form</li>";
echo "<li>Test contributions pages</li>";
...
echo "<ul>";
echo "<li><strong>Missing Library:</strong> Added module_lib loading to Partner controller</li>";
echo "<li><strong>View Context:</strong> Pass module_lib to all views via \$data array</li>";
echo "<li><strong>Method Access:</strong> hasModule() and other methods now available in views</li>";
echo "</ul>";

echo "<h3>✅ Code Changes:</h3>";
echo "<ul>";
echo "<li>Added: <code>\$this->load->library('module_lib');</code> to constructor</li>";
echo "<li>Added: <code>\$data['module_lib'] = \$this->module_lib;</code> to all view methods</li>";
echo "<li>Result: Student header can access module_lib methods</li>";
echo "<li>Result: No more 'Undefined property' errors</li>";
echo "</ul>";
echo "</div>";

echo "<p><strong>Partner module_lib fix completed!</strong> The Partner pages should now load without any module_lib-related errors.</p>";
?>
